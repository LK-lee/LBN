import userModel from "../models/roles.model.js";
...
const addrole = (req, res) => {
    try {
        ...
        if (!useremail) {
            ...
            userModel.addrole(userData, (err, result) => {
                if (err) {
...
const getrolebyid = (req,res) => {
    ...
    userModel.getrolebyid(id,(err,result)=>{
        if(err){
            return res.status(400).json({errors: err.message});
        }else{
            res.status(200).json({
                status: "success",
                message: "role fetched by id sucessfully",
                data: result[0]
            })
        }
    })
...
const updaterole = (req,res) => {
    ...
    userModel.updaterole(id,userData,(err,result)=>{
        if(err){
            return res.status(400).json({errors: err.message});
        }
        userModel.getuserbyemail(useremail,(err,userresult)=>{
...
const deleterole = (req, res) => {
    ...
    userModel.getrolebyid(id,(err,result)=>{
        ...
        if(!result || result.length === 0){
            return res.status(400).json({errors: "role not found"});
        }
        userModel.deleterole(id,(err,result1)=>{
            if(err){
                return res.status(400).json({errors: err.message});
            }userModel.getuserbyemail(useremail, (err, userresult) => {
                if (err) {
...
export {addrole, getroles, getrolebyid, updaterole, deleterole};